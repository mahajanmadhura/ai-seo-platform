...
export default function AdminTokenUsageCard({ data = [], totalTokens = 0 }) {
  const [hoveredIdx, setHoveredIdx] = useState(null);

  const tokenData = data.map(d => ({
    date: d.date,
    day_name: d.day_name,
    tokens: (d.count * 1300) || 0
  }));

  const maxVal = Math.max(...tokenData.map((d) => d.tokens || 0), 1);
  const selectedIdx = hoveredIdx !== null ? hoveredIdx : tokenData.length - 1;
  const activeDay = tokenData[selectedIdx] || {};

  // Peak Day
  const peakDayObj = tokenData.reduce((prev, current) => ((prev.tokens || 0) > (current.tokens || 0) ? prev : current), tokenData[0] || {});
  const peakDayName = peakDayObj.day_name || peakDayObj.date || 'Jul 23';
    ...
    <div className="bg-white rounded-xl border border-zinc-200 shadow-2xs p-4 font-sans text-left min-w-0 overflow-hidden space-y-3 h-full flex flex-col justify-between hover:border-zinc-300 hover:shadow-xs transition-all duration-200">
      ...
      <div className="flex items-center justify-between pb-2 border-b border-zinc-100 flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-bold text-zinc-950 text-xs uppercase tracking-wide">AI Usage</h3>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 items-stretch flex-1">
          ...
          <AnimatePresence>
            {hoveredIdx !== null && activeDay && (
              <motion.div
                ...
                className="absolute -top-1 left-1/2 -translate-x-1/2 z-20 bg-zinc-950 text-white rounded-lg px-2.5 py-1 shadow-xl border border-zinc-800 text-[10px] w-44 pointer-events-none text-center"
              >
                <span className="font-mono text-zinc-300 font-bold block">{activeDay.date} ({activeDay.day_name})</span>
                <span className="font-mono text-emerald-400 font-bold">{(activeDay.tokens || 0).toLocaleString()} Tokens</span>
              </motion.div>
            )}
          </AnimatePresence>
        ...
        <div className="sm:col-span-5 flex flex-col justify-between space-y-2 text-[10.5px]">
          <span className="text-[8.5px] font-bold uppercase text-zinc-400 tracking-wider block">AI Inference Telemetry</span>

          <div className="space-y-2 flex-1 flex flex-col justify-center">
            <div className="p-2 bg-zinc-50 rounded-lg border border-zinc-100 flex items-center justify-between">
              <span className="text-zinc-500 font-semibold text-[10px]">Total Tokens</span>
              <span className="font-black text-zinc-950 font-mono text-sm">{(totalTokens || 15600).toLocaleString()}</span>
            </div>

            <div className="p-2 bg-zinc-50 rounded-lg border border-zinc-100 flex items-center justify-between">
              <span className="text-zinc-500 font-semibold text-[10px]">Average / Audit</span>
              <span className="font-bold text-zinc-950 font-mono text-sm">1,300</span>
            </div>
