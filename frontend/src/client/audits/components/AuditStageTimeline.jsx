...
const STEPS = [
  { key: 'QUEUED', label: 'Queuing crawler parameters' },
  { key: 'FETCHING_WEBSITE', label: 'Fetching website headers' },
  { key: 'CHECKING_ROBOTS', label: 'Checking sitemap & robots.txt' },
  { key: 'CRAWLING_PAGES', label: 'Crawling page structure' },
  { key: 'CHECKING_LINKS', label: 'Testing internal links' },
  { key: 'PAGESPEED', label: 'Analyzing performance speed' },
  { key: 'SAVING_ISSUES', label: 'Saving audits & recommendations' },
...
export default function AuditStageTimeline({ currentStep, status }) {
  ...
  return (
    <div className="bg-white rounded-3xl p-6 border border-border-color/60 shadow-sm text-left">
      ...
      <div className="relative pl-6 border-l-2 border-border-color/30 space-y-6">
        {STEPS.map((step, index) => {
          ...
          const isFailed = status === 'FAILED' && step.key === currentStep;
            ...
            <div key={step.key} className="relative flex items-center gap-4">
              <div className="absolute -left-[33px] flex items-center justify-center">
                {isCompleted ? (
                  ...
                  <div className="w-5 h-5 bg-white border-2 border-red-500 rounded-full flex items-center justify-center shadow-sm relative">
              ...
              <div className="flex-1">
                <p className={`text-xs font-bold transition-colors duration-300 ${
                  isActive ? 'text-deep-green' : isFailed ? 'text-red-500 font-bold animate-pulse' : isCompleted ? 'text-muted-text/80 font-semibold' : 'text-muted-text/50 font-medium'
                }`}>
                  {step.label} {isFailed && '(Failed)'}
                </p>
              </div>
            </div>
          );
