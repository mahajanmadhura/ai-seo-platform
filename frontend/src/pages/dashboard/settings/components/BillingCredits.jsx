...
export default function BillingCredits() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-border-color/60 shadow-sm">
        <div className="border-b border-border-color/40 pb-4 mb-5">
          <h3 className="font-black text-deep-green text-base tracking-tight flex items-center gap-2">
            <FileText className="w-4.5 h-4.5 text-[#0B5A4A]" /> Transaction Log
          </h3>
          <p className="text-[10px] text-muted-text mt-0.5 font-semibold">Your invoices and credit usages.</p>
        </div>

        {billingLoading ? (
          <div className="space-y-3">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="h-12 bg-soft-bg rounded-xl animate-pulse"></div>
            ))}
          </div>
                ...
                {Array.isArray(transactions) && transactions.map((tx) => {
                  ...
                  return (
                    <tr key={tx.id} className="hover:bg-soft-bg/30">
                      <td className="py-3 pl-2 font-medium text-muted-text">
                        {tx.created_at ? new Date(tx.created_at).toLocaleString() : 'N/A'}
                      </td>
                      <td className="py-3 uppercase text-[9px] font-black tracking-wider">
                        <span
                      ...
                      <td className="py-3 text-deep-green font-semibold">{tx.description}</td>
                      <td className={`py-3 text-right pr-2 font-black ${isPositive ? 'text-[#0B5A4A]' : 'text-red-600'}`}>
